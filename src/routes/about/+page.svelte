...
<section class="py-25 px-4">
    ...
    <div id="vision" class="max-w-3xl mx-auto mb-16 p-8 bg-gradient-to-br from-gray-900/80 to-black/80 backdrop-blur-sm rounded-lg shadow-2xl border border-cyan-500/30 relative z-10 group hover:scale-[1.02] transition-transform duration-300">
        ...
        <p class="text-gray-100 mb-4 leading-relaxed">
            My vision is to bridge the gap between complex technology and intuitive user experiences. I am committed to developing software solutions that are not only aesthetically pleasing but are built upon a foundation of clean, efficient, and scalable code. I aim to create accessible digital ecosystems that deliver real value to users.
        </p>
        <p class="text-gray-100 mb-4 leading-relaxed">
            I believe that powerful technology carries the responsibility to drive positive innovation. I am committed to writing clean code and building accessible applications that truly matter. My focus is on integrating strong backend architecture with seamless frontend design to engineer reliable solutions within this endless digital web. 
        </p>
    </div>
    ...
    <div id="journeySection" class="max-w-3xl mx-auto bg-gradient-to-b from-gray-900/80 to-black/80 backdrop-blur-sm rounded-lg shadow-2xl overflow-hidden border border-gray-700/50 relative z-10 ">
        <nav class="flex border-b-2 border-red-600/50 bg-black/50">
            <button
                ...
                class="flex-1 px-6 py-4 text-center font-semibold transition-all duration-300 {activeTab === 'education' ? 'bg-gradient-to-r from-red-600 to-red-700 text-white' : 'text-gray-300 hover:text-cyan-400 hover:bg-gray-800/50'}"
            >
                Education
            </button>
            <button
                on:click={() => changeTab('organization')}
                class="flex-1 px-6 py-4 text-center font-semibold transition-all duration-300 {activeTab === 'organization' ? 'bg-gradient-to-r from-red-600 to-red-700 text-white' : 'text-gray-300 hover:text-cyan-400 hover:bg-gray-800/50'}"
            >
                Organization
            </button>
            <button
                on:click={() => changeTab('achievement')}
                class="flex-1 px-6 py-4 text-center font-semibold transition-all duration-300 {activeTab === 'achievement' ? 'bg-gradient-to-r from-red-600 to-red-700 text-white' : 'text-gray-300 hover:text-cyan-400 hover:bg-gray-800/50'}"
            >
                Achievement
            </button>
        </nav>
                ...
                <div class="space-y-6">
                    ...
                    {#each organizationData as org}
                        <div class="bg-gray-800/40 p-6 rounded-lg shadow-lg border border-red-500/20 hover:border-cyan-400/40 transition-all duration-300 hover:shadow-red-500/20 hover:scale-[1.02] transition-transform duration-300">
                            ...
                            <p class="text-gray-300">{org.description}</p>
                        </div>
                    {/each}
                </div>
            {/if}
